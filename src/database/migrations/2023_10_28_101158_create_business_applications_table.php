...
return new class () extends Migration {
    ...
    public function up(): void
    {
        Schema::create('business_applications', function (Blueprint $table) {
            $table->comment('事業者申込申請');

            // IDや認証に関連するカラム
            $table->increments('business_application_id')->comment('事業者申込ID');
            $table->string('email', 2048)->unique()->comment('事業者メールアドレス: 暗号化');

            // ユーザー情報に関連するカラム
            $table->integer('corporation_id')->nullable()->comment('法人ID: 親法人が存在する場合に必要');
            $table->string('corporation_name', 100)->nullable()->comment('法人名');
            $table->string('applicant_name', 2048)->nullable()->comment('申込者氏名: 暗号化');
            $table->string('applicant_name_kana', 2048)->nullable()->comment('申込者氏名（カナ）: 暗号化');
