...
return new class () extends Migration {
    ...
    public function up(): void
    {
        Schema::create('transfer_requests', function (Blueprint $table) {
            ...
            $table->string('transaction_period_from')->comment('取引期間FROM');
            $table->string('transaction_period_to')->comment('取引期間TO');

            // 銀行関連のカラム
            $table->string('confirm_bank_name', 255)->nullable()->comment('振込確定時点の金融機関名: 暗号化');
            $table->string('confirm_branch_name', 255)->nullable()->comment('振込確定時点の支店名: 暗号化');
            $table->string('confirm_account_type', 255)->nullable()->comment('振込確定時点の口座種別: 1:普通預金口座 2:当座預金口座, 暗号化');
            $table->string('confirm_account_number', 255)->nullable()->comment('振込確定時点の口座番号: 暗号化');
            $table->string('confirm_account_holder_name', 255)->nullable()->comment('振込確定時点の口座名義: 暗号化');
