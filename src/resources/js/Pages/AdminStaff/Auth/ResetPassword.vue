...
const isUnexpectedError = computed<boolean>(() => {
  const { email, token } = form.errors;
  if (email || token) return true;
  return false;
});

const submit = () => {
  form.post(route(RouteName.AdminStaffPasswordStore), {
  ...
  >
    <div class="p-staff-reset-password">
      <div class="p-staff-reset-password__container">
        <Alert v-if="isUnexpectedError" text="予期せぬエラーが発生しました" class="p-staff-reset-password__error" />
