...
import { useForgotPassword } from '@/Hooks/BusinessOperator/useForgotPassword';
...
<template>
  <BaseLayout title="パスワードを忘れた方">
    <HeaderSubText text="パスワードを忘れた方" :href="RouteName.BusinessOperatorLogin" />
    <Divider />
    <div class="p-business-operator-forgot-password">
      <div class="p-business-operator-forgot-password__container">
        <Paragraph class="p-business-operator-forgot-password__message">
          パスワード再設定メールを送信します。 スタッフ登録時に登録されたメールアドレスを 入力してください。
        </Paragraph>
        <form class="p-business-operator-forgot-password__form" @submit.prevent="onSubmit">
          <FormGroupInput
            id="email"
            label="メールアドレス"
            ...
            class="p-business-operator-forgot-password__email"
          />
          <Button class="p-business-operator-forgot-password__submit" :disabled="form.processing" text="送信" />
          <ColorSection :is-rounded="true">
            <Paragraph>
              ※URLの有効期限は60分です。※登録時のメールアドレスが、変更等により利用できない場合は、ご本人確認ができないためパスワードの再発行ができません。その場合は、お手数ですが、
              <Link text="新規会員登録" :href="RouteName.BusinessOperatorPasswordRequest" />
              をお願いいたします。
...
<style lang="scss">
.p-business-operator-forgot-password {
  &__container {
    padding: 24px 16px;
  }
  &__message {
