...
const form = useForm<{ businessId: number; mediaId: number | string }>({
...
const deleteMedia = (mediaId: number) => {
  if (window.confirm('本当に削除しますか？')) {
    ...
    form.delete(route(RouteName.CorporationMediaCheckDelete));
  }
};
</script>
...
<template>
  ...
  >
    <div class="p-business-operator-image-check">
      <EmptySection v-if="props.dateList.length == 0" text="投稿している動画・画像はございません。" />
      <div
