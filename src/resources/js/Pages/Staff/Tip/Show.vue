...
const onDelete = (): void => {
  ...
  deleteForm.delete(
    route(RouteName.StaffTipsDelete, { tipId: props.userTip.tipId, replyId: props.staffTipReply.replyId }),
  );
};

const onSubmit = (): void => {
  form.post(route(RouteName.StaffTipsCreate, { tipId: props.userTip.tipId }), {
    onSuccess: () => {
      form.reset();
